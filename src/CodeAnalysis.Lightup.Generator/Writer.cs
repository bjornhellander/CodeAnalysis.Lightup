...
{{
    ...
    {{
        ...
        public readonly object{na} Unwrap()
            => wrappedObject;
...
";
        ...
        string nullableAnnotation)
    {
        ...
        if (typeDef is EnumTypeDefinition enumTypeDef)
        {
            ...
            {
                return GenerateNewEnum(enumTypeDef, targetNamespace, nullableAnnotation);
            }
            else if (typeDef.IsUpdated)
            {
                return GenerateUpdatedEnum(enumTypeDef, typeDefs, targetNamespace, nullableAnnotation);
            }
            else
            {
                return null;
        ...
        string nullableAnnotation)
    {
        ...
        foreach (var property in instanceProperties)
        {
            sb.AppendLine();
            AppendMemberSummary(sb, property);
            sb.AppendLine($"        public readonly {GetPropertyTypeDeclText(property, nullableAnnotation, typeDefs)} {property.Name}");
            sb.AppendLine($"        {{");
        ...
        foreach (var methodDef in instanceMethods)
        {
            ...
            sb.AppendLine();
            AppendMemberSummary(sb, methodDef);
            sb.AppendLine($"        public readonly {GetMethodReturnTypeDeclText(methodDef, nullableAnnotation, typeDefs)} {methodDef.Name}({GetParametersDeclText(methodDef.Parameters, nullableAnnotation, typeDefs)})");
