...
internal class Writer
{
    ...
    {
        [ParameterMode.None] = "",
        [ParameterMode.In] = "", // NOTE: Avoiding 'in' to make the code compatible with c# 6
...
";
        ...
        context.AddSource(targetFilePath, SourceText.From(NormalizeNewLine(source), Encoding.UTF8));
    }
...
";
        ...
        TypeReference typeRef)
    {
        if (typeRef is GenericTypeReference genericTypeRef)
        {
            ...
            else if (IsValueTask(genericTypeRef) && !useValueTaskType)
            {
    ...
    private static bool IsValueTask(GenericTypeReference genericTypeRef)
    {
        if (genericTypeRef.OriginalType is not NamedTypeReference namedTypeRef)
        {
            return false;
    ...
    [SuppressMessage("MicrosoftCodeAnalysisCorrectness", "RS1035:Do not use APIs banned for analyzers", Justification = "Ok")]
